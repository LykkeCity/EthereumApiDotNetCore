//В этом контракте даны коментарии на русском языке, о его функциональности
//MarsTokenV1 - представляет из себя централизованный ерс20 совместимый смартконтракт, 
//Из основных минусов - записи балансов хранятся в отдельном смартконтракте BalanceSheet,
//Этот смартконтракт можно заменить в любое время на новый смартконтракт функцией setBalanceSheet(address _sheet).
//Все переводы MarsTokenV1 могут быть заморожены, MarsTokenV1 расширяет Pausable.
//Адреса держателей могут быть добавлены в черный список, а затем их баланс может быть стерт, MarsTokenV1 расширяет Blacklistable
//
//
//
